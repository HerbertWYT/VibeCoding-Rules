...
治理文件把工程约束收敛为唯一权限链和 `AUDIT → SPEC_DRAFT → EXECUTING / FROZEN` 门禁：工程行为必须由用户要求、当前已批准 SDD、TDD 和当前证据驱动，模型不得通过自己补写文档获得实施权。缺少 SDD 或 TDD 时必须先辅助建立；新增功能和行为变化必须在编码前全局更新 SDD/TDD，不能追加与旧定义并存的补丁条款；Bug 修复必须先判断是实现违反正确 SDD，还是 SDD 本身缺失或错误，前者保持规范正文稳定并增加回归 TDD 和追溯记录，后者先全局修正规范。实现、测试和交付汇报必须引用准确的用户来源、SDD/TDD ID、实现位置和当前证据；子工程、框架、依赖和架构不能由技术理由自我授权；本地 Git 必须隔离上级、下级和兄弟仓库。
